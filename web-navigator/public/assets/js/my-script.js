...
function ExportReport(id, url, query_str, total_urls) {
    ...
    html += '<option value="CAD">CAD Canadian</option>';
    html += '</select>';
    html += '</div>';
    html += '</div>';
    //
...
//for sales team
$("#sales_add_domain").submit(function(e) {
    e.preventDefault();
    g_loader("on", "Processing...");
    let domain = $.trim($("#domain").val());
    var parser = document.createElement('a');
    if(domain !=''){
        parser.href = domain;
        domain = parser.protocol + "//" + parser.hostname;
    ...
    $.ajax({
        data: { domain: domain },
        url: "/navibot/web-navigator/sales/add-domain",
        type: "POST",
        dataType: 'json',
        cache: false,
        success: function(data) {
            ...
            if (data.status != undefined) {
                if (data.status == 'success') {
                    swal({
                        ...
                        if (e.value) {
                            window.location.href = "/navibot/web-navigator/sales/dashboard";
                        }
                    })
                } else {
...
//
function salesExport(id){
    //alert(id);
    g_loader("on", "Processing...");
    $("#export_cancel").click();
    $.ajax({
        type: 'POST',
        url: "/navibot/web-navigator/sales/export-report/", // + domain_id + "/" + h + "/" + m + "/" + resource,
        data: {
            domain_id: id,
        },
        dataType: 'json'
